...
//func PrintPoints(QRpionts &qrCapstone) {
//	fmt.Printf("cap.c:  %v \n", QRpionts)
//
//}
...
func (q *recognizer) recordCapstone(ring, stone int) {
	...
	//getting center coner of each coner squer
	//fmt.Printf("Center %v  \n", &capstone.center)
	//fmt.Printf("grid %v  \n", &q.grids)
...
func perspectiveMap(c []float64, u, v float64, ret *point) {
	...
	ret.x = int(x + 0.5)
	ret.y = int(y + 0.5)
	//fmt.Printf("den test %v  \n", den)
	//fmt.Printf("persp x test %v  \n", ret.x)
	//fmt.Printf("persp y test %v  \n", ret.y)
}
